...
import { getAccessToken, parseQuery, stringQuery } from '~/common/utils';
...
class RevGet extends React.Component {
  ...
  pageChangeList = () => {
    ...
    const qparams = parseQuery(history.location.search);
    delete qparams.revid;
    history.push(`${history.location.pathname.replace('revGet', 'revList')}?${stringQuery(qparams)}`);
  }

  apiRevUpdStatus = (revid, status) => async () => {
    const { rev, history } = this.props;
    await rev.apiRevUpd({
      revMap: {
        revid,
        ...
        <div className={classes.btnWrap}>
          ...
          {revInfo.status === 'RR' && (
            <Button
              color="info"
              disabled={false}
              onClick={this.apiRevUpdStatus(revInfo.revid, 'RM')}
            >
              <Icon icon={check} /> 예약금 입금 대기
            </Button>
          )}
          {revInfo.status === 'RM' && (
            <Button
              color="info"
              disabled={false}
              onClick={this.apiRevUpdStatus(revInfo.revid, 'RC')}
            >
              <Icon icon={check} /> 예약완료
            </Button>
          )}
          {revInfo.status !== 'CC' && (
            <Button
              color="danger"
              disabled={false}
              onClick={this.apiRevUpdStatus(revInfo.revid, 'CC')}
            >
              <Icon icon={x} /> 예약취소
            </Button>
          )}
          {revInfo.status !== 'FF' && (
            <Button
              color="danger"
              disabled={false}
              onClick={this.apiRevUpdStatus(revInfo.revid, 'FF')}
            >
              <Icon icon={x} /> 예약불가
            </Button>
          )}
