...
import { parseQuery, getAccessToken, stringQuery } from '~/common/utils';
...
class RevList extends React.Component {
  ...
  pageChange = () => (e, offset, page) => {
    ...
    history.push({
      pathname: match.url,
      search: stringQuery({ currentPageNo: page }),
    });
  };
  ...
  pageChangeGet = (item) => () => {
    ...
    qparams.revid = item.revid;
    history.push(`${history.location.pathname.replace('revList', 'revGet')}?${stringQuery(qparams)}`);
  }
  ...
  render() {
    ...
    const revList = toJS(rev.getRevList.get('ADM'));
    const qparams = parseQuery(history.location.search);
    return (
